...
class Solution:
    ...
    def coinChange(self, coins: list[int], amount: int) -> int:
        minWaysToMake = defaultdict(lambda: math.inf)
        minWaysToMake[0] = 0
        coins.sort()
        ...
        for i in range(1, amount + 1):
            for coin in coins:
                if coin > i: break
                remains = i - coin
                if remains not in minWaysToMake: continue
                ways = 1 + minWaysToMake[remains]
                minWaysToMake[i] = min(ways, minWaysToMake[i])

        return int(minWaysToMake[amount]) if amount in minWaysToMake else -1
